...
import {
  buildArchitectPanel,
  buildElementPanelFromSchema,
} from "./PanelBuilder.mjs";
...
Architect.prototype.outputJSON = function () {
  ...
  const data =
    ...
    encodeURIComponent(JSON.stringify(this.schema, null, 2));
...
Architect.prototype.onConstructorUpdate = function (id, newConstructorKey) {
  ...
  if (!component)
    throw new Error(
      `Component type ${newConstructorKey} not found in componentsMap`
    );
  component.id = id;
  ...
  if (!parentSchema && !key && schema) {
    ...
    {
      let { parentSchema, key, schema } = getSchemaDataFromId(
        id,
...
export function getSchemaDataFromId(id, rootSchema) {
  if (rootSchema.id === id)
    return { parentSchema: null, key: null, schema: rootSchema };
  ...
  for (let key in rootSchema.subComponents) {
    if (rootSchema.subComponents[key].id === id) {
      return {
        ...
        key: key,
        schema: rootSchema.subComponents[key],
      };
    }
