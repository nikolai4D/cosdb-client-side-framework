...
export function Component(options = {}) {
  ...
  this.slot = function (component) {
    ...
    try {
      key = Object.keys(this.subComponents).find(
        (key) => this.subComponents[key] === component
      );
    } catch (e) {
      console.error(
        "Error while finding key for component: ",
        component,
        "while trying to fill slot",
        "key: ",
        key,
        "subComponents keys: ",
  ...
  this.deSetComponentsFromString = function () {
    if (!Object.values(this.subComponents).every((v) => this.isString(v)))
      return;
    for (const [key, value] of Object.entries(this.subComponents)) {
  ...
  this.findComponentDataById = function (id) {
    if (this.id === id)
      return { parentComponent: null, key: null, foundComponent: this };
    else {
      for (let key in this.subComponents) {
        let component = this.subComponents[key];
        if (component === null)
          return { parentComponent: this, key: key, foundComponent: null };
        if (component.id === id)
          return { parentComponent: this, key: key, foundComponent: component };
        else {
